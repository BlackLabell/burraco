...
console.log('--- Pausa online (4 settembre 2026) ---');

t('richiediPausa registra una richiesta verso la pausa, e blocca le mosse mentre è in sospeso', () => {
  const g = E.newGame('1v1', { seed: 600 });
  eq(g.paused, false); eq(g.pausaRichiesta, null);
  const r = E.richiediPausa(g, 0);
  assert(r.ok);
  assert(g.pausaRichiesta && g.pausaRichiesta.da === 0 && g.pausaRichiesta.versoPausa === true,
    'la richiesta deve segnare chi la fa e che va verso la pausa (non si era ancora in pausa)');
  eq(g.paused, false, 'la richiesta da sola non mette ancora in pausa: serve la risposta');
});

t('non si può aprire una seconda richiesta mentre una è già in sospeso, da nessuno dei due lati', () => {
  const g = E.newGame('1v1', { seed: 601 });
  assert(E.richiediPausa(g, 0).ok);
  assert(!E.richiediPausa(g, 1).ok, 'non deve poterne aprire una seconda mentre la prima aspetta risposta');
  assert(!E.richiediPausa(g, 0).ok, 'nemmeno chi ha già chiesto può richiederla di nuovo');
});

t('solo l\'altro posto può rispondere: chi ha chiesto non può rispondere a se stesso', () => {
  const g = E.newGame('1v1', { seed: 602 });
  E.richiediPausa(g, 0);
  assert(!E.rispondiPausa(g, 0, true).ok, 'chi ha fatto la richiesta non può accettarla da solo');
  assert(E.rispondiPausa(g, 1, true).ok, 'solo l\'altro posto può rispondere');
});

t('rispondiPausa con sì mette davvero in pausa, e pulisce la richiesta in sospeso', () => {
  const g = E.newGame('1v1', { seed: 603 });
  E.richiediPausa(g, 0);
  const r = E.rispondiPausa(g, 1, true);
  assert(r.ok);
  eq(g.paused, true);
  eq(g.pausaRichiesta, null, 'la richiesta deve sparire una volta risposta, in un senso o nell\'altro');
});

t('rispondiPausa con no lascia tutto com\'era', () => {
  const g = E.newGame('1v1', { seed: 604 });
  E.richiediPausa(g, 0);
  const r = E.rispondiPausa(g, 1, false);
  assert(r.ok);
  eq(g.paused, false, 'una richiesta di pausa rifiutata non deve mettere in pausa');
  eq(g.pausaRichiesta, null);
});

t('mentre è in pausa, pescare/calare/scartare vengono tutti rifiutati dal motore', () => {
  const g = E.newGame('1v1', { seed: 605 });
  E.richiediPausa(g, 0);
  E.rispondiPausa(g, 1, true);
  assert(g.paused);
  const p = g.turn;
  assert(!E.draw(g, p, 'stock').ok, 'draw deve rifiutare mentre è in pausa');
  // qualcosa in mano per provare anche meld/discard, a prescindere da g.phase
  assert(!E.meldNew(g, p, []).ok, 'meldNew deve rifiutare mentre è in pausa');
  assert(!E.discard(g, p, g.hands[p][0].id).ok, 'discard deve rifiutare mentre è in pausa');
});

t('dopo la pausa si può richiedere la ripresa, sempre con la risposta dell\'altro, e il gioco riparte', () => {
  const g = E.newGame('1v1', { seed: 606 });
  E.richiediPausa(g, 0);
  E.rispondiPausa(g, 1, true);
  assert(g.paused);
  const r1 = E.richiediPausa(g, 1);   // stavolta è l'altro posto a muoversi per primo
  assert(r1.ok);
  assert(g.pausaRichiesta.versoPausa === false, 'partendo da in pausa, la richiesta deve andare verso la ripresa');
  assert(!E.rispondiPausa(g, 1, true).ok, 'ancora: chi ha chiesto non può rispondersi da solo');
  const r2 = E.rispondiPausa(g, 0, true);
  assert(r2.ok);
  eq(g.paused, false, 'con la risposta sì, la partita deve tornare giocabile');
  const p = g.turn;
  assert(E.draw(g, p, 'stock').ok, 'dopo la ripresa le mosse devono tornare valide');
});

t('richiediPausa/rispondiPausa passano da applicaMossa, come ogni altra mossa del registro online', () => {
  const g = E.newGame('1v1', { seed: 607 });
  let r = E.applicaMossa(g, { t: 'pq', p: 0 });
  assert(r.ok);
  eq(g.pausaRichiesta.da, 0);
  r = E.applicaMossa(g, { t: 'pa', p: 1, accetta: true });
  assert(r.ok);
  eq(g.paused, true, 'la coppia di mosse pq/pa applicate via applicaMossa deve mettere in pausa');
});

t('richiediPausa rifiuta se la partita è già finita', () => {
  const g = E.newGame('1v1', { seed: 608, target: 1 });   // traguardo bassissimo: finisce alla prima mano
  g.finished = true;   // equivalente a una partita già chiusa
  assert(!E.richiediPausa(g, 0).ok, 'non ha senso mettere in pausa una partita già finita');
});
